...
## Phase 54: AccordionTrigger 구조 개선 - 이벤트 버블링 완전 해결 (완료)

- [x] AccordionTrigger 컴포넌트 제거
- [x] 커스텀 헤더 구조로 변경 (체크박스/라벨 영역과 화살표 버튼 분리)
- [x] 체크박스/라벨 영역과 화살표 토글 버튼 완전히 분리
- [x] 화살표 버튼만 아코디언 상태 제어
- [x] 체크박스 클릭 시 이벤트 버블링 완전 차단
- [x] 모든 테스트 통과 (59개)
- [x] 빌드 에러 없음

**구현 내용:**
- 개인정보 수집 및 이용 동의: AccordionTrigger 제거 → 커스텀 헤더 구조
- 마케팅 정보 수신 동의: AccordionTrigger 제거 → 커스텀 헤더 구조
- 광고성 정보 수신 동의: AccordionTrigger 제거 → 커스텀 헤더 구조
- 화살표 버튼: onClick 이벤트로 직접 아코디언 상태 토글
- 체크박스/라벨: 이벤트 버블링 완전 차단으로 아코디언 상태 변경 방지

**동작:**
- 전체 동의 체크 시: 아코디언 펼침 없이 체크만 됨
- 개별 동의 체크 시: 아코디언 펼침 없이 체크만 됨
- 화살표 버튼 클릭 시: 아코디언 펼침/닫힘 (토글)
