...
- [ ] DB 스키마 수정: status, deleted_at 컬럼 추가
- [ ] API 수정: soft delete 필터링 적용
- [ ] Admin 컴포넌트: 테이블 UI 구현 (카드형 → 테이블형)
- [ ] 검색 기능: 회사명, 담당자, 연락처, 이메일
- [ ] 필터 기능: 진행 현황별 필터 (전체, 진행중, 타겟처, 가망처, 수주완료, DROP)
- [ ] 정렬 기능: 접수일 정렬 (최신순/오래된순)
- [ ] 진행 현황 드롭다운: 5가지 상태 (진행중, 타겟처, 가망처, 수주완료, DROP)
- [ ] 상태 색상 배지: 파랑, 주황, 초록, 보라, 빨강
- [ ] 삭제 기능: soft delete + 확인 모달
- [ ] CSV 다운로드 기능
- [ ] 모바일 대응: 가로 스크롤
- [ ] 상담신청/자료요청 공통 적용
- [ ] 최종 검증 및 배포
