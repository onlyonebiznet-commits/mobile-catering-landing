                ...
                <form onSubmit={handleSubmit} className="space-y-3">
                  ...
                  <div className="space-y-3 border-t border-gray-200 pt-3" aria-label="개인정보 동의">
                    ...
                    <Accordion type="single" collapsible value={accordionValue} onValueChange={setAccordionValue} className="w-full space-y-2">
                      {([
                        ['personal-info','chatbot-personal-info','개인정보 수집 및 이용 동의', 'personalInfoCollection', 'CJ프레시웨이㈜는 서비스 상담 제공을 위해 아래와 같이 개인정보를 수집·이용합니다. 수집한 개인정보는 목적 이외의 용도로 처리하지 않으며, 처리 목적을 변경할 경우 별도로 안내하고 동의를 받겠습니다. 내용을 자세히 읽으신 후 동의 여부를 결정하여 주시기 바랍니다. (필수) 개인정보 수집 및 이용｜수집·이용 목적: 서비스 상담 신청 접수, 상담 진행｜수집 항목: 성명, 기업명, 연락처, 이메일주소, 지역, 예상 식수｜보유·이용 기간: 상담일로부터 1개월｜(선택) 상담 내용 관련 정보｜수집·이용 목적: 상담 내용 파악 및 맞춤 서비스 제안｜수집 항목: 관심 서비스, 이용 환경, 요청사항(문의 내용)｜보유·이용 기간: 상담일로부터 1개월｜* 귀하는 개인정보 수집 및 이용에 대한 동의를 거부하실 권리가 있습니다. 다만 필수항목에 대한 동의를 거부하실 경우 서비스 상담 신청이 제한됩니다. 선택항목은 입력하지 않으셔도 서비스 상담 이용에 제한이 없습니다.'],
                      ...
                      ] as const).map(([value, id, label, key, description]) => <AccordionItem key={value} value={value} className="rounded-[10px] border border-gray-200 px-3"><div className="flex items-center justify-between py-2"><div className="flex min-w-0 items-center gap-2"><Checkbox id={id} checked={agreements[key]} onCheckedChange={(checked) => handleAgreementChange(key, checked === true)} onClick={stopAgreementPropagation} onPointerDown={stopAgreementPropagation} /><Label htmlFor={id} className="form-checkbox-label cursor-pointer text-gray-700" onClick={stopAgreementPropagation} onPointerDown={stopAgreementPropagation}>{label}{key === 'personalInfoCollection' && <span className="text-status-error" aria-hidden="true"> *</span>}</Label></div><button type="button" className="flex-shrink-0 rounded-[10px] p-1 text-gray-500 hover:bg-gray-100" onClick={() => toggleAgreementAccordion(value)} aria-label={`${label} 내용 열기`} aria-expanded={accordionValue === value}><ChevronDown className={`h-4 w-4 transition-transform ${accordionValue === value ? 'rotate-180' : ''}`} aria-hidden="true" /></button></div><AccordionContent className="rounded-[10px] bg-gray-50 p-3 text-xs leading-relaxed text-gray-600">{description}</AccordionContent></AccordionItem>)}
                    </Accordion>
                  </div>
