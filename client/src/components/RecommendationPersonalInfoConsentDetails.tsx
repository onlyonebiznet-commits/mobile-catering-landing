...
type RecommendationPersonalInfoConsentDetailsProps = HTMLAttributes<HTMLDivElement>;

/**
 * The approved personal-information notice for the service recommendation
 * widget. Its collection fields differ from the main consultation form.
 */
export function RecommendationPersonalInfoConsentDetails({
  className = "",
  ...props
}: RecommendationPersonalInfoConsentDetailsProps) {
  return (
    <div
      className={`recommendation-personal-info-consent-details space-y-4 text-[8px] leading-relaxed text-gray-600 md:text-xs ${className}`}
      {...props}
    >
      <p>
      ...
      <section className="space-y-2" aria-labelledby="recommendation-required-personal-info-title">
        <h3
          id="recommendation-required-personal-info-title"
          className="text-[18px] font-semibold text-gray-900 md:text-[24px]"
        >
        ...
        <div className="grid grid-cols-3 border border-gray-300 bg-white text-left">
          ...
          <div className="border-r border-t border-gray-300 px-2 py-3">이름, 회사명·기관명, 연락처, 이메일, 예상 식수, 희망 지역</div>
          <div className="border-t border-gray-300 px-2 py-3">상담일로부터 1개월</div>
        </div>
      </section>

      <section className="space-y-2" aria-labelledby="recommendation-optional-consultation-info-title">
        <h3
          id="recommendation-optional-consultation-info-title"
          className="text-[18px] font-semibold text-gray-900 md:text-[24px]"
        >
